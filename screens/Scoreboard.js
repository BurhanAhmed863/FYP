...
const Scoreboard = ({ route }) => {
  ...
  const handleOverComplete = async () => {
    try {
      ...
      const response = await fetch(
        `${apiConnection.apiIp}/inningsScoreboardData.php`,
        {
          method: 'POST',
          headers: {
        ...
        const processBattingData = matchData => {
          ...
          matchData.forEach(item => {
            if (!batsmanMap[item.batsman_name]) {
              ...
              if (
                item.runs_scored > batsmanMap[item.batsman_name].runs_scored
              ) {
                batsmanMap[item.batsman_name] = item;
              }
            }
  ...
  const updateScoreSecondInnings = (
    runs,
    isWicket = false,
    isExtra = false,
  ) => {
      ...
      if (isWicket) {
        if (selectedBatsman.name === selectedBatsman1.name) {
          ...
          setSelectedBatsman1(prevBatsman2ndInnings => {
            if (
              innings == 2 &&
              (!prevBatsman2ndInnings || !prevBatsman2ndInnings.hasReset)
            ) {
              return {
                hasReset: true,
              };
          ...
          if (isExtra || isWicket) {
            if (legalBallsInOver <= 4) {
              if (innings == 1) {
                ...
                console.log('1-ball No', legalBallsInOver);
                setCurrentOverAndBallForFirstInnings(
                  newOvers + '.' + legalBallsInOver,
                );
              }
            }
            if (legalBallsInOver === 5 || isWicket) {
              if (innings == 1) {
                ...
                // runsInOver.push(runs); // Store the runs for the legal ball
                legalBallsInOver++; // Increment only for legal balls
                console.log('2-ball No', legalBallsInOver);
                setCurrentOverAndBallForFirstInnings(
                  newOvers + '.' + legalBallsInOver,
                );
        ...
        if (innings == 1) {
          if (isExtraType !== 'LegByes' || isExtra !== 'Byes' || isWicket) {
              ...
              if (legalBallsInOver <= 4) {
                if (isWicket) {
                  ...
                  if (innings == 1) {
                    ...
                    runsInOver.push(runs); // Store the runs for the legal ball
                    legalBallsInOver++; // Increment only for legal balls
                    console.log('3-ball No', legalBallsInOver);
                    setCurrentOverAndBallForFirstInnings(
                      newOvers + '.' + legalBallsInOver,
                    );
                  ...
                  if (innings == 1) {
                    ...
                    runsInOver.push(runs); // Store the runs for the legal ball
                    legalBallsInOver++; // Increment only for legal balls
                    console.log('4-ball No', legalBallsInOver);
                    setCurrentOverAndBallForFirstInnings(
                      newOvers + '.' + legalBallsInOver,
                    );
                    wicket = wicket + 1;
              ...
              if (legalBallsInOver === 5) {
                if (isWicket) {
                  ...
                  if (innings == 1) {
                    ...
                    // runsInOver.push(runs); // Store the runs for the legal ball
                    legalBallsInOver++; // Increment only for legal balls
                    console.log('5-ball No', legalBallsInOver);
                ...
                if (innings == 1) {
                  ...
                  // runsInOver.push(runs); // Store the runs for the legal ball
                  legalBallsInOver++; // Increment only for legal balls
                  console.log('6-ball No', legalBallsInOver);
            ...
            if (isOverComplete || (isOverComplete && isWicket)) {
              ...
              console.log(
                'Sixes and Fours',
                selectedBatsman1.sixes,
                selectedBatsman1.fours,
              );
              // sendBatsmenDataToBackend(overNumber);
              newBalls = 0;
              legalBallsInOver = 0; // Reset after over completion
  ...
  const updateBowlerStatsForSeconInnings = (
    runs,
    isWicket = false,
    isExtra = false,
    isExtraType = null,
            ...
            if (sendResultToAPICondition == true) {
              const sendResultToAPI = async resultMessage => {
                try {
                  ...
                  const response = await fetch(
                    `${apiConnection.apiIp}/results.php`,
                    {
                      method: 'POST',
                      headers: {
                        'Content-Type': 'application/json',
              ...
              if (totalRunsSecondInnings > totalRunsFirstInnings + 1) {
                ...
                totalRunsSecondInnings < totalRunsFirstInnings + 1 &&
  ...
  const addOverDetails = (ballNumber, runs, overNumber) => {
    ...
    setRunsInOver(prevRunsInOver => {
      ...
      const totalRunsFirstInnings = updatedRuns.reduce(
        (acc, run) => acc + run,
        0,
      );
  ...
  const handleNoBallRunUpdate = (extraType, isExtraType) => {
    ...
    if (extraType === 'None') {
      ...
      if (innings == 2) {
        updateBowlerStatsForSeconInnings(
          runs + 1,
          false,
          true,
          isExtraType,
    ...
    if (
      isExtraType == 'NoBall' &&
      (extraType === 'Byes' || extraType === 'Legbyes')
    ) {
      if (innings == 1) {
  ...
  const renderByesLegByesModal = () => (
    <Modal
      visible={isByesLegByesModalVisible}
      transparent
      animationType="slide">
      <View style={ScoreboardStyle.modalContainer}>
        <RNPickerSelect
  ...
  async function fetchMatchData(matchId) {
    ...
    try {
      const response = await fetch(
        `${apiConnection.apiIp}/getMatchDetails.php`,
        {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
      ...
      if (data.status === 'success') {
        if (innings == 1) {
          ...
          const validBatsmen = (data.batting_team || [])
        ...
        if (innings == 2) {
          ...
          // console.log('Bowling First:', data.bowlingFirst);
          // Process and filter players and bowlers, removing null or undefined data
          const validBatsmen = (data.bowling_team || [])
            .filter(player => player && player.name)
            .map(player => ({
              name: player.name || 'Unknown Player',
              id: player.id,
  ...
  const saveMatchDataToBackend = async (
    overNumber,
    ballInOver,
    totalBalls,
    runsInOver,
    ...
    if (innings == 1) {
      if (
        (currentOverAndBallForFirstInnings && totalOvers) !== 0 &&
        currentOverAndBallForFirstInnings === totalOvers
      ) {
        ...
        const matchData = {
          ...
          economy_rate: runsInOver
            .reduce((acc, run) => acc + run, 0)
            .toFixed(2),
        ...
        try {
          const response = await fetch(
            `${apiConnection.apiIp}/matchOversData.php`,
            {
              method: 'POST',
              headers: {
                'Content-Type': 'application/json',
    ...
    if (innings == 2) {
      if (
        (currentOverAndBallForSecondInnings && totalOvers) !== 0 &&
        currentOverAndBallForSecondInnings === totalOvers
      ) {
        if (bowlingTeamId === null) {
          console.error('Bowling First is not set!');
          return;
  ...
  const AvsB =
    colors.background === '#333'
      ? require('../assets/icons/AvB.png')
      : require('../assets/icons/AvB.png');
  const userIcon =
    colors.background === '#333'
      ? require('../assets/icons/user_white.png')
  ...
  return (
    <ScrollView
      ...
      <Modal
        ...
        <View style={ScoreboardStyle.modalContainer}>
          <View style={ScoreboardStyle.modalContent}>
            <Text style={ScoreboardStyle.modalHeader}>
            ...
            <RNPickerSelect
              onValueChange={value =>
                handleBowlerSelection(
                  bowlers.find(bowler => bowler.name === value),
                )
              }
        ...
        <>
          <View style={ScoreboardStyle.header}>
            ...
            <Icon
              ...
              size={23}
              color="#f79e1b"
              style={ScoreboardStyle.iconSearch}
            />
          </View>
              ...
              <View style={ScoreboardStyle.batterText}>
                  ...
                  <TouchableOpacity
                    ...
                    {selectedBatsman ? (
                      ...
                      <View
                        ...
                        <Text style={ScoreboardStyle.batTxt}>
                      ...
                      <View
                        style={[
                          ...
                          {
                            backgroundColor:
                              selectedBatsman === selectedBatsman1.name
                                ? 'orange'
                                : 'grey',
                            flex: 1,
                            textAlign: 'left',
                ...
                <View
                  ...
                  <Text style={ScoreboardStyle.runs}>
                    {selectedBatsman1.runs}
                  </Text>
                  <Text style={ScoreboardStyle.runs}>
                    {selectedBatsman1.balls}
                  </Text>
                  <Text style={ScoreboardStyle.runs}>
                    {selectedBatsman1.fours}
                  </Text>
                  ...
                  <Text style={ScoreboardStyle.runs}>
                    {selectedBatsman1.strikeRate.toFixed(2)}
                  </Text>
                </View>
              </View>
            )}
            ...
            {selectedBatsman2 && (
              <View style={ScoreboardStyle.batterText}>
                    ...
                    {selectedBatsman ? (
                      ...
                      <View
                        style={[
                          ...
                          {
                            backgroundColor:
                              selectedBatsman.name === selectedBatsman2.name
                                ? 'orange'
                                : 'grey',
                            flex: 1,
                            textAlign: 'left',
                      ...
                      <View
                        style={[
                          ...
                          {
                            backgroundColor:
                              selectedBatsman === selectedBatsman2.name
                                ? 'orange'
                                : 'grey',
                            flex: 1,
                            textAlign: 'left',
                ...
                <View
                  ...
                  <Text style={ScoreboardStyle.runs}>
                    {selectedBatsman2.runs}
                  </Text>
                  <Text style={ScoreboardStyle.runs}>
                    {selectedBatsman2.balls}
                  </Text>
                  <Text style={ScoreboardStyle.runs}>
                    {selectedBatsman2.fours}
                  </Text>
                  <Text style={ScoreboardStyle.runs}>
                    {selectedBatsman2.sixes}
                  </Text>
                  <Text style={ScoreboardStyle.runs}>
                    {selectedBatsman2.strikeRate.toFixed(2)}
                  </Text>
                </View>
              </View>
            )}
                ...
                <View
                  ...
                  {innings == 1 && (
                    <View style={{ flexDirection: 'row' }}>
                      ...
                      <Text style={ScoreboardStyle.runs}>
                        {selectedBowler.maiden}
                      </Text>
                      <Text style={ScoreboardStyle.runs}>
                        {selectedBowler.runs}
                      </Text>
                      <Text style={ScoreboardStyle.runs}>
                        {selectedBowler.wicket}
                      </Text>
                      <Text style={ScoreboardStyle.runs}>
                        {selectedBowler.economy?.toFixed(1) ?? 0.0}
                      </Text>
                    </View>
                  )}
                  {innings == 2 && (
              ...
              <Modal
                ...
                <View style={ScoreboardStyle.modalContainer}>
                  <View style={ScoreboardStyle.modalContent}>
                    ...
                    {/* Wicket Type Options */}
                    {['Bowled', 'Caught', 'LBW', 'Stumped', 'Run Out'].map(
                      type => (
                        <TouchableOpacity
                          key={type}
                          onPress={() => {
                  ...
                  <View style={ScoreboardStyle.modalContent}>
                    ...
                    {selectedBatsman && (
                      <View style={ScoreboardStyle.batsmanDetails}>
                        <Text style={ScoreboardStyle.runsH}>
                          Batsman: {selectedBatsman.name}
                        </Text>
                        <Text style={ScoreboardStyle.runsH}>
                          Runs: {selectedBatsman.runs}
                        </Text>
                        <Text style={ScoreboardStyle.runsH}>
                          Balls: {selectedBatsman.balls}
                        </Text>
                        <Text style={ScoreboardStyle.runsH}>
                          4s: {selectedBatsman.fours}
                        </Text>
                        <Text style={ScoreboardStyle.runsH}>
                          6s: {selectedBatsman.sixes}
                        </Text>
                        <Text style={ScoreboardStyle.runsH}>
                          SR: {selectedBatsman.strikeRate.toFixed(2)}
                        </Text>
                      </View>
                    )}
