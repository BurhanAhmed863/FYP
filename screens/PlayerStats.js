...
const PlayerStats = ({route, navigation}) => {
  ...
  const fetchBattingStats = async playerId => {
    try {
      const response = await fetch(
        `${apiConnection.apiIp}/fetchPlayerStats.php?playerId=${playerId}`,
      );
      ...
      if (result.status === 'success') {
        ...
        const batting = {
          ...
          notOuts:
            (parseInt(data.total_matches, 10) || 0) -
  ...
  const fetchBowlingStats = async playerId => {
    try {
      const response = await fetch(
        `${apiConnection.apiIp}/fetchBowlerStats.php?playerId=${playerId}`,
      );
      const text = await response.text();
      console.log(text);
      const result = JSON.parse(text);
      ...
      if (result.status === 'success') {
        ...
        const bowling = {
          ...
          wides: parseInt(data.total_wide_balls, 10) || 0, // ✅ White ball ka data handle kiya
          noBall: parseInt(data.total_no_balls, 10) || 0,
        };
  ...
  return (
    <ScrollView
      style={[MatchesStyle.container, {backgroundColor: colors.background}]}>
      ...
      <View style={styles.toggleContainer}>
        <TouchableOpacity
          style={[
            styles.toggleButton,
            view === 'batting' && styles.activeButton,
          ]}
        ...
        <TouchableOpacity
          style={[
            styles.toggleButton,
            view === 'bowling' && styles.activeButton,
          ]}
